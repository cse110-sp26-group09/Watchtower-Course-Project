(function () {
  ...
  var dashboardModeSelect = document.getElementById("dashboard-mode-select");
  var dashboardModePill = document.getElementById("dashboard-mode-pill");
  ...
  var managerSummaryList = document.getElementById("manager-summary-list");
  var developerRouteTable = document.getElementById("developer-route-table");
  var developerVersionTable = document.getElementById("developer-version-table");
  var developerEventMix = document.getElementById("developer-event-mix");
  var developerRoutePressure = document.getElementById("developer-route-pressure");
  var developerTopIssues = document.getElementById("developer-top-issues");
  var developerLatencyWindows = document.getElementById("developer-latency-windows");
  var developerTrafficPeaks = document.getElementById("developer-traffic-peaks");
  var developerIssueSearchInput = document.getElementById("developer-issue-search");
  var developerMuteToggleButton = document.getElementById("developer-mute-toggle");
  var developerMuteStatusLine = document.getElementById("developer-mute-status");
  var developerCriticalCount = document.getElementById("dev-critical-count");
  var developerWarningCount = document.getElementById("dev-warning-count");
  var developerInfoCount = document.getElementById("dev-info-count");
  var developerTotalCount = document.getElementById("dev-total-count");
  ...
  var DASHBOARD_MODE_STORAGE_KEY = "watchtower_dashboard_mode";
  ...
  function applyDashboardMode(modeValue) {
    var resolvedMode = getDashboardModeValue(modeValue);
    ...
    document.querySelectorAll("[data-dashboard-mode]").forEach(function (modePanel) {
      var panelMode = getDashboardModeValue(modePanel.getAttribute("data-dashboard-mode"));
      modePanel.hidden = panelMode !== resolvedMode;
    });
  }

  function initializeDashboardModeControl() {
    var savedMode = loadUiPreference(DASHBOARD_MODE_STORAGE_KEY);
  ...
  function renderManagerSummary(stats, activityEvents) {
    ...
    var topFeature = deriveFeatureCounts(activityEvents || [])[0];
    var routeNames = Object.keys((stats && stats.latencyByRoute) || {});
  ...
  function renderDeveloperHomeDiagnostics(stats) {
    if (developerRouteTable) {
      var routeNames = Object.keys(stats.latencyByRoute || {}).sort(function (leftRoute, rightRoute) {
        return ((stats.latencyByRoute[rightRoute] || {}).p95 || 0) - ((stats.latencyByRoute[leftRoute] || {}).p95 || 0);
      });
      ...
      if (routeNames.length === 0) {
        ...
        developerRouteTable.innerHTML = routeNames.slice(0, 6).map(function (routeName) {
          var routeStats = stats.latencyByRoute[routeName] || {};
    ...
    if (developerVersionTable) {
      var versionKeys = Object.keys(stats.errorsByVersion || {}).sort(function (leftVersion, rightVersion) {
  ...
  function renderDeveloperInsights(stats, activityEvents) {
    var errors = (stats.recentErrors || []).slice(0, 5);
    ...
    if (developerTopIssues) {
      if (errors.length === 0) {
        ...
        developerTopIssues.innerHTML = errors.map(function (eventRecord) {
          var message = eventRecord && eventRecord.data && eventRecord.data.message ? eventRecord.data.message : 'Unknown error';
    ...
    if (developerTrafficPeaks) {
      var hourCounts = {};
      (activityEvents || []).forEach(function (eventRecord) {
        var timestamp = getValidTimestamp(eventRecord && eventRecord.timestamp);
        if (!timestamp) return;
        var hourKey = new Date(timestamp).toISOString().slice(0, 13);
        hourCounts[hourKey] = (hourCounts[hourKey] || 0) + 1;
      });

      var topHours = Object.keys(hourCounts).sort(function (leftHour, rightHour) {
      ...
      if (topHours.length === 0) {
        ...
        developerTrafficPeaks.innerHTML = topHours.map(function (hourKey) {
          var label = new Date(hourKey + ':00:00Z').toLocaleTimeString([], { hour: 'numeric' });
  ...
  function renderDeveloperAnalyticsDiagnostics(rangeEvents, rangeLatencySummary) {
    if (developerEventMix) {
      ...
      (rangeEvents || []).forEach(function (eventRecord) {
        var eventType = eventRecord && eventRecord.type ? String(eventRecord.type) : 'unknown';
        typeCounts[eventType] = (typeCounts[eventType] || 0) + 1;
      });

      var eventTypes = Object.keys(typeCounts).sort(function (leftType, rightType) {
    ...
    if (developerRoutePressure) {
      var routeNames = Object.keys(rangeLatencySummary || {}).sort(function (leftRoute, rightRoute) {
      ...
      if (routeNames.length === 0) {
        ...
        developerRoutePressure.innerHTML = routeNames.slice(0, 6).map(function (routeName) {
          var routeStats = rangeLatencySummary[routeName] || {};
          return '<li><span>' + escapeHtml(routeName) + '</span><strong>' + String(routeStats.p95 || 0) + ' ms</strong></li>';
        }).join('');
      }
    }
