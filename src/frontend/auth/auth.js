...
(() => {
  ...
  // When served by the WatchTower backend the dashboard is the stable /dashboard
  // route; opened directly from disk it lives in the sibling dashboard/ folder.
  const servedByServer = window.location.protocol !== "file:";
  const DASHBOARD_URL = servedByServer
    ? new URL("/dashboard", window.location.href).href
    : new URL("../dashboard/index.html", window.location.href).href;
