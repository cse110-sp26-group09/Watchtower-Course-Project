...
npm run config:clerk          # writes src/frontend/auth/clerk-config.js
```
...
and [`docs/adr/ADR-0006.md`](../../../docs/adr/ADR-0006.md).
...
- `auth.js` derives the Clerk CDN from the key, loads the browser SDK, and:
  ...
  - redirects to the dashboard (`/dashboard`) after auth (and immediately if a
