 ...
 *     (set in /login/clerk-config.js, loaded before this file).
...
(() => {
  ...
  const LOGIN_URL = new URL("/login/", window.location.href).href;
  ...
  if (
    ...
    console.warn(
      "[auth-guard] Missing Clerk publishable key. Redirecting to login. " +
        "Ensure /login/clerk-config.js is served before auth-guard.js."
    );
    redirectToLogin();
    return;
  }
