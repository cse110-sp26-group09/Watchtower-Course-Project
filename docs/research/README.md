# Research

This is all of the research, planning notes, and supporting documentation that we have compiled and consolidated. 

We mainly focused on...
- Alerting and uptime tracking
- Dashboard and analytics UX
- Error tracking and deployment monitoring
- User needs, personas, and product direction

## Contents

### User Personas & Product Notes
User stories, stakeholder needs, and user persona research.
[Woosik Research
](docs/research/Woosik-research.md)
[Jason Research
](docs/research/Jason-research.md)
[Hemendra Research
](docs/research/Hemendra-research.md)

This is what we plan on using to come up with product decisions, features, dashboard design, and workflows.
